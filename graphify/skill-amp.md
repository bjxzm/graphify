...
/graphify <path> --excel                              # also export graph.xlsx with labels, nodes, relations, sources
...
**Generate HTML always** (unless `--no-viz`). Generate Excel only if `--excel` was explicitly given, and generate an Obsidian vault only if `--obsidian` was explicitly given.
...
If `--excel` was given, export the source-traceable workbook before cleanup:

```bash
graphify export excel
# or with custom path: graphify export excel --output /path/to/graph.xlsx
```

The first sheet matches the six-column data-labeling layout (`序号`, `时间`, `主要标签`, `次要标签`, `关联标签`, `内容`). The remaining sheets preserve nodes, relationships, communities, sources, and export metadata. See `references/exports.md` for the field mapping and safety rules.
...
Tell the user (omit the Excel line unless --excel was given, and omit the obsidian line unless --obsidian was given):
```
Graph complete. Outputs in PATH_TO_DIR/graphify-out/
  ...
  graph.xlsx            - labeled tables + provenance (only if --excel was given)
...
When the user asks to research a topic, classify the task, build a query plan, search authoritative sources first, extract identifiers/entities, fan out for a second round, and stop based on evidence-role coverage rather than result count. Use host search for the open web and the Python pipeline for deterministic validation, ranking, coverage, and review. See `references/discovery.md`; never ingest pending candidates without explicit approval.
