...
def _run_cli() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("  discover <topic>        find and rank source candidates; never ingests before review")
        print("    --provider P            arxiv or crossref (repeatable; both by default)")
        print("    --feed URL              add an RSS/Atom provider (repeatable)")
        print("    --input JSON            import host-AI candidates (repeatable)")
        print("    --profile PROFILE       auto|general|government-program")
        print("    --plan-only             print the research plan without network access")
        print("    --domain-cap D=N        override per-domain cap (repeatable)")
        print("    --role-quota R=N        reserve evidence-role coverage (repeatable)")
        print("    --out PATH              candidate queue (default graphify-out/discovery/candidates.json)")
        print("  collect                 ingest explicitly approved candidates in a saved queue")
        print("    --approve 1,3           approve queue positions or stable candidate IDs")
        print("    --approve-all           explicitly approve the whole queue")
        print("    --dry-run               show approvals without ingesting")
        print("  schedule init|run|status automate discovery from graphify-sources.json")
        print("    schedule run --loop     keep polling; automatic ingest remains opt-in by score")
        ...
        print("  export excel            emit a source-traceable graph.xlsx workbook")
