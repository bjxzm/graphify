...
If `--excel` was given, run `graphify export excel` before cleanup (or add `--output /path/to/graph.xlsx`). The first sheet matches the six-column data-labeling layout (`序号`, `时间`, `主要标签`, `次要标签`, `关联标签`, `内容`); the remaining sheets preserve nodes, relationships, communities, sources, and export metadata.
...
Tell the user (omit the Excel line unless --excel was given; omit the obsidian line unless --obsidian was given; omit the wiki line unless --wiki was given):
```
